...
const FilterForm = ({ onFilter }) => {
  ...
  const priceOptions = [
    { value: '', label: 'To $', isDisabled: true }, // Опция "To $" добавлена как строка по умолчанию
    { value: 30, label: '30' },
    { value: 40, label: '40' },
    { value: 50, label: '50' },
    { value: 60, label: '60' },
  ...
  const [selectedBrand, setSelectedBrand] = useState(null);
  const [selectedPrice, setSelectedPrice] = useState(priceOptions[0]); // Устанавливаем первую опцию (To $) как значение по умолчанию
  const [mileageFrom, setMileageFrom] = useState(''); // Состояние для "From"
  const [mileageTo, setMileageTo] = useState(''); // Состояние для "To"
